...
int validPointer(PtABPNode pointer);
...
{
	if (validPointer(*proot)){
	
		if ((*proot)->PtLeft == NULL){
			PtABPNode min = *proot;
			*proot = (*proot)->PtRight;
			ABPNodeDestroy (&min);	/* eliminar o elemento - deleting the node */
		}
		else {
...
{ 
	if (validPointer(*proot)){
		...
		if ((*proot)->PtRight == NULL){
			PtABPNode max = *proot;
			*proot = (*proot)->PtLeft;
			ABPNodeDestroy (&max);	/* eliminar o elemento - deleting the node */
		}
		else {
...
{	
	...
	if (validPointer(proot)){
		if (proot->Elem < pvalue){
			total = 1 + ABPSize(proot->PtLeft) + ABPRank(proot->PtRight, pvalue);			
		}
		else {
			total = ABPRank(proot->PtLeft, pvalue);
		}
		Error = OK;
	} else {
		Error = ABP_EMPTY;
...
void ABPElements (PtABPNode proot, PtQueue pqueue, int plow, int phigh)
{
	if (validPointer(proot)){
		...
		// search left first to garantee order
		if (proot->Elem > plow)
			ABPElements(proot->PtLeft, pqueue, plow, phigh);
		
		// check value and add to queue if in interval
...
{ 
	...
	int flag;
	if (validPointer(proot)){
		flag = ABPElement(ABPMinRep(proot)) % 2;
	}
	else {
		return 0;
	}
	...
	if ((pstack = StackCreate (sizeof(int))) == NULL){
	...
	while(proot != NULL || StackIsEmpty(pstack) == 0) { // breaks if pointer is null and stack empty
...
int validPointer(PtABPNode node){
	if (node == NULL) {
		Error = ABP_EMPTY;
		return 0;
	}
