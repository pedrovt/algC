/*******************************************************************************
 ...
 Nome : Pedro Veloso Teixeira                                       NMEC : 84715
...
void ABPDeleteMin (PtABPNode *proot) { 
	// Uma função recursiva para apagar o elemento mínimo armazenado na árvore.
	/* Finding the minimum element in a Binary Search Tree
	 * implies recursevely visiting the left "side" of the tree until the reaching
	 * the minimum node
	 */
	...
	// Min is root --> we need to update the tree root
	if (node -> PtLeft == NULL) {
		*proot = node -> PtRight;
		ABPNodeDestroy(&node);
		Error = OK;
	...
	// Found the min node
	if (node -> PtLeft == NULL) {
		PtABPNode minNode = *proot;
		node = (node)->PtRight;
		ABPNodeDestroy(&minNode);
		Error = OK;
	}
	else {
...
PtABPNode ABPFloorValue (PtABPNode proot, int pvalue) { 
	/* função repetitiva */
	// Uma função repetitiva para obter um ponteiro para o nó do elemento pvalue ou, caso ele não exista na árvore, para o nó do maior dos elementos menores do que ele. Se esse nó não existir, deverá ser devolvido o ponteiro nulo.
	...
	while (proot != NULL) {
		...
		if (proot -> Elem < pvalue && proot -> PtRight == NULL) {
			return proot;
		}
...
PtABPNode ABPCeilValue (PtABPNode proot, int pvalue) { 
	/* função recursiva */
	// Uma função recursiva para obter um ponteiro para o nó do elemento pvalue ou, caso ele não exista na árvore, para o nó do menor dos elementos maiores do que ele. Se esse nó não existir, deverá ser devolvido o ponteiro nulo.
	...
	if (proot->Elem > pvalue && ((proot -> PtLeft -> Elem < pvalue) || (proot -> PtLeft == NULL))) {
		return proot;
	}
...
int ABPRank (PtABPNode proot, int pvalue) { 
	/* função recursiva */
	// Uma função recursiva para determinar o número de elementos inferiores a pvalue armazenados na árvore – sugestão : será útil usar a função ABPSize já existente no tipo de dados ABP.
	...
	if (proot-> Elem < pvalue) {
		rank = 1 + ABPSize(proot->PtLeft) + ABPRank(proot->PtRight, pvalue);
	}
	else {
		rank = ABPRank(proot->PtLeft, pvalue);
	}
...
void ABPElements (PtABPNode proot, PtQueue pqueue, int plow, int phigh) { 
	/* função recursiva */
	// Uma função recursiva para registar ordenadamente, numa fila de inteiros já existente e referenciada pelo ponteiro pqueue, os elementos da árvore pertencentes ao intervalo fechado [plow, phigh].
	...
	// Go left first (order must be assured)
	if (proot -> Elem > plow) {
		ABPElements(proot->PtLeft, pqueue, plow, phigh);
	}

	// check value of the node and add to queue if in the wanted interval
...
int ABPIsEvenOdd (PtABPNode proot) { 
	/* função repetitiva */
	// Uma função repetitiva para determinar se a árvore é alternadamente composta por números pares e ímpares ou ímpares e pares, fazendo uma travessia em ordem, recorrendo a uma pilha.Tenha em atenção que a função – após as validações necessárias – deve começar por determinar a paridade do elemento mínimo da árvore.
	...
	// a função – após as validações necessárias – deve começar por determinar a paridade do elemento mínimo da árvore.
	int isEvenOdd = ABPElement(ABPMinRep(proot)) % 2;
	

	// travessia em ordem recorrendo a uma pilha
	PtStack pstack = StackCreate(sizeof(int));
	...
	StackPush(pstack, &proot);
	proot = proot -> PtLeft;
	PtABPNode pnode = NULL;

	// travessia em ordem
	while (proot != NULL || StackIsEmpty(pstack) == 0) { 
		...
		if (proot != NULL) {
			...
			StackPop(pstack, &pnode);
			if (isEvenOdd != (pnode -> Elem % 2)) {
				StackDestroy(&pstack);
				return 0;
			}
			isEvenOdd = !isEvenOdd;
